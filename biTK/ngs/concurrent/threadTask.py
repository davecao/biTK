...
class Task(object):
    ...
    def __init__(self, taskid, func, *args, **kwargs):
        """
        Args:
            taskid(str): task id
...
class ThreadTask(TaskWithEvent):
    ...
    def __init__(self, taskid, func, args=(), kwargs={}, callback=None):

        super(ThreadTask, self).__init__(taskid, func, *args, **kwargs)
        #self.callback = kwargs.pop("callback", None)
    ...
    def run(self):
        ...
        try:
            if self.func is None:
                ...
                self.returned_value = self.func(*self.args, **self.kwargs)
                self._callback(self.returned_value)

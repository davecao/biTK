...
#--- Quantile ---
def clip(data, min_, max_):
    """clip a list with (min,max) """
    if iter(data) is data:
        data = list(data)
    n = len(data)
    if n < 1:
        raise StatisticsError('clip requires at least one data point')
    data = [ max(min_, min(n, max_)) for n in data]
    return data

def quantile(a, prob=list([.25,.5,.75]), alphap=.4, betap=.4):
    """ Return empirical quantiles for a list.
    ...
    for inx, val in enumerate(m):
        aleph = n*prob[inx] + val
        # clip aleph [1, n-1] and int
        k_val = int(math.floor(max(1, min(aleph, n-1))))
        # clip aleph -k [0, 1]
        gamma_val = max(0, min((aleph - k_val),1))
        #k.append(k_val)
        #gamma.append(gamma_val)
        r = (1.0-gamma_val)*x[k_val-1] + gamma_val*x[k_val]
        result.append(r)
    return result
